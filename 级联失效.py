...
for beta in np.arange(0,1.5,0.1):
    ...
    for failnode in [11,17]:
        ...
        edge_weight = {}
        for edge in G.edges():
            edge_weight[edge] = (G.nodes[edge[0]]['weight'] + G.nodes[edge[1]]['weight'])/2
        ...
        node_capacity = {}
        for node in G.nodes():
            node_capacity[node] = (1+beta)*G.nodes[node]['initial']
        ...
        while len(A) != 0:
            for node in A:
                ...
                for neb in nx.neighbors(G, node):
                    if (G.nodes[node]['initial'] * (G.edges[node, neb]['weight']
                        /G.out_degree(node, weight='weight')) +  G.nodes[neb]['initial']
                        > G.nodes[neb]['capacity']):
                        if G.nodes[neb]['state'] == 'S':
                            G.nodes[neb]['state'] = 'F'
                            A.append(neb)
                if t == 1:
                    NS = len(A)
