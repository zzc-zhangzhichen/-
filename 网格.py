...
filepath = r"C:\Users\86183\Desktop\百度API\lanzhou\\"  # 设置存放路径
    ...
    for i in range(0, len(roads)):
        ...
        if len(x['road_traffic'][0]) == 1:
            ...
            for j in range(len(congestions)):
                traffic = [x['road_traffic'][0]['road_name'], date, h, m, evaluation_status, evaluation_status_desc,
                           congestions[j]['section_desc'], congestions[j]['status'], congestions[j]['congestion_trend'],
                           congestions[j]['congestion_distance'], congestions[j]['speed']]
        print(traffic)
