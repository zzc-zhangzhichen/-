...
def remove_edge(G,edge):
    ...
    if C_total_neighbors == 0:
        ...
        x = (G.edges[edge]['L0']) / (C_total_neighbors )
        for k in nb:
            G.edges[k]['L0'] += G.edges[k]['C0'] * x    # 将被毁节点的负载，按容量大小比例分配给相邻节点
    print(str(edge) + ' , 长度为: ' + str(G.edges[edge]['D']))
